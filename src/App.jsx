...
export default function App() {
  ...
  const [editingMessage, setEditingMessage] = useState(null); // { timestamp, text }
  ...
  const searchGif = async (query) => {
    setGifLoading(true);
    try {
      // Tenor v2 API — key demo publik (rate limit longgar untuk dev)
      const res = await fetch(
        `https://tenor.googleapis.com/v2/search?q=${encodeURIComponent(query || 'trending')}&key=AIzaSyB9k5NHsYjBF8K3VToFMH4NJsBDjOXAevE&limit=20&media_filter=gif`
      );
      const data = await res.json();
      setGifResults(data.results || []);
    } catch (e) {
      showNotif("Gagal memuat GIF", "error");
    }
    setGifLoading(false);
  };
  ...
  const sendGif = async (gifUrl) => {
    ...
    try {
      const res = await fetch(`${API_URL}/chats/${user.id}/${activeChatFriend.id}`, {
        ...
        body: JSON.stringify({ senderId: user.id, text: '', gif: gifUrl })
      });
  ...
  // chat edit message
  const saveEditMessage = async (timestamp, newText) => {
    if (!newText.trim() || !activeChatFriend) return;
    try {
      await fetch(`${API_URL}/chats/${user.id}/${activeChatFriend.id}/${timestamp}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    } catch (e) { showNotif("Gagal edit pesan", "error"); }
  };
  ...
  // --- SINKRONISASI UMUM ---
  useEffect(() => {
    // Tambahkan di dalam useEffect yang sudah ada untuk activeChatFriend:
            ...
            .then(r => r.json()).then(d => {
              if (isMounted && d && Array.isArray(d.messages)) {
                setChatMessages(prev =>
                  prev.length !== d.messages.length ? d.messages : prev
                );
                // Tandai sebagai dibaca setiap kali data baru masuk
                markAsRead();
              }
            }).catch(() => { });
        };
    ...
    if (activeChatFriend && user) {
      const fetchChat = () => {
        fetch(`${API_URL}/chats/${user.id}/${activeChatFriend.id}`)
          .then(r => r.json()).then(d => {
            if (isMounted && d && Array.isArray(d.messages)) {
              setChatMessages(prev => prev.length !== d.messages.length ? d.messages : prev);
            }
          }).catch(() => { });
      };
      fetchChat();
  ...
  const handleLogin = async (e) => {
    ...
    const payload = userType === 'admin'
      ...
      : { is_admin: false, nisn: loginForm.nisn.trim() };
    try {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col font-sans pb-20 md:pb-0 text-gray-200">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed top-[57px] left-0 right-0 z-40 bg-gray-900/98 backdrop-blur-xl border-b border-gray-800 shadow-2xl animate-slideDown">
          <div className="p-4 space-y-1">
            {/* Info profil di menu */}
            <div className="flex items-center gap-3 bg-gray-800 p-3 rounded-2xl mb-3 border border-gray-700" onClick={() => { setCurrentView('profile'); setMobileMenuOpen(false); }}>
              ...
              <div className="flex-grow">
                ...
                <p className="text-[9px] font-bold text-blue-400 uppercase tracking-widest">{user.level !== 'Admin' ? `Level ${calculateLevel(user.xp)} • ${user.xp} XP` : 'Admin MAX'}</p>
              </div>
              <span className="text-[9px] bg-blue-600 text-white px-2 py-1 rounded-lg font-black uppercase">Profil</span>
            </div>
      ...
      {/* KONTEN UTAMA */}
      <main className="flex-grow p-4 md:p-8 max-w-7xl mx-auto w-full pt-[72px] md:pt-8">
            ...
            <div className="bg-gray-900 border border-gray-800 p-6 md:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="text-center md:text-left">
                  <h1 className={`text-3xl md:text-4xl font-black italic uppercase tracking-tighter leading-none mb-2 ${myAchs.glowName ? 'text-white drop-shadow-[0_0_10px_rgba(59,130,246,0.8)]' : 'text-white'}`}>Halo, {(user.name || '').split(' ')[0]}!</h1>
                ...
                <>
                  ...
                  <div className="flex-grow overflow-y-auto p-4 md:p-6 flex flex-col gap-6 scrollbar-thin relative bg-gray-900">
                    ...
                    {(chatMessages || []).map((msg, idx) => {
                      ...
                      const isLastFromMe = isMe && idx === chatMessages.length - 1;
                      const isRead = isLastFromMe && (msg.readBy || []).includes(activeChatFriend?.id);
                      const isEditing = editingMessage?.timestamp?.toString() === msg.timestamp?.toString();

                      return (
                        <div key={idx} className={`flex gap-3 items-end ${isMe ? 'justify-end' : 'justify-start'}`}>
                          {!isMe && <SafeAvatar src={avatarSrc} sizeClassName="w-8 h-8" />}
                          <div className="flex flex-col group relative max-w-[75%] md:max-w-md">

                            {/* Bubble pesan */}
                            <div className={`rounded-2xl shadow-sm ${msg.gif ? 'p-1 overflow-hidden bg-transparent border border-gray-700' :
                              isMe ? 'p-3 px-4 bg-blue-600 text-white rounded-br-sm' :
                                'p-3 px-4 bg-gray-800 text-gray-200 border border-gray-700 rounded-bl-sm'
                              }`}>
                              {msg.gif
                                ? <img src={msg.gif} alt="GIF" className="rounded-xl max-w-[200px] max-h-[200px] object-cover" />
                                : isEditing
                                  ? (
                                    <form onSubmit={(e) => { e.preventDefault(); saveEditMessage(msg.timestamp, editingMessage.text); }} className="flex gap-2 items-center">
                                      <input
                                        autoFocus
                                        value={editingMessage.text}
                                        onChange={e => setEditingMessage(prev => ({ ...prev, text: e.target.value }))}
                                        className="bg-blue-700 text-white rounded-lg px-2 py-1 text-sm outline-none border border-blue-400 flex-grow"
                                      />
                                      <button type="submit" className="text-white bg-green-500 rounded-lg p-1"><Check size={14} /></button>
                                      <button type="button" onClick={() => setEditingMessage(null)} className="text-white bg-gray-600 rounded-lg p-1"><X size={14} /></button>
                                    </form>
                                  )
                                  : <p style={{ wordBreak: 'break-word', whiteSpace: 'pre-wrap' }} className="text-sm">{msg.text}</p>
                              }
                            </div>

                            {/* Tombol aksi (muncul saat hover) */}
                            {isMe && !msg.gif && (
                              <div className="absolute top-1/2 -translate-y-1/2 -left-20 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                  onClick={() => setEditingMessage({ timestamp: msg.timestamp, text: msg.text })}
                                  className="text-gray-500 hover:text-blue-400 p-1.5 bg-gray-900 rounded-lg border border-gray-700"
                                ><Edit size={13} /></button>
                                <button
                                  onClick={() => deleteChatMessage(msg.timestamp)}
                                  className="text-gray-500 hover:text-red-500 p-1.5 bg-gray-900 rounded-lg border border-gray-700"
                                ><Trash2 size={13} /></button>
                              </div>
                            )}

                            {/* Timestamp + status edited + read receipt */}
                            <div className={`flex items-center gap-1 mt-1 ${isMe ? 'justify-end' : 'justify-start'}`}>
                              {msg.edited && !msg.gif && (
                                <span className="text-[8px] text-gray-500 italic">diedit</span>
                              )}
                              {msg.timestamp && (
                                <span className="text-[9px] opacity-50 text-gray-400">
                                  {new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </span>
                              )}
                              {/* Read receipt — hanya tampil di pesan terakhir milik kita */}
                              {isLastFromMe && (
                                <span className={`text-[9px] font-bold ${isRead ? 'text-blue-400' : 'text-gray-600'}`}>
                                  {isRead ? '✓✓' : '✓'}
                                </span>
                              )}
                            </div>
                  ...
                  {/* GIF Picker Panel */}
                  {showGifPicker && (
                    <div className="absolute bottom-[72px] left-0 right-0 bg-gray-950 border-t border-gray-800 p-3 z-20 shadow-xl">
                      <div className="flex gap-2 mb-3">
                        <input
                          placeholder="Cari GIF..."
                          value={gifSearch}
                          onChange={e => setGifSearch(e.target.value)}
                          onKeyDown={e => e.key === 'Enter' && searchGif(gifSearch)}
                          className="flex-grow bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-xs outline-none focus:border-blue-500"
                          autoFocus
                        />
                        <button onClick={() => searchGif(gifSearch)} className="bg-blue-600 text-white px-3 py-2 rounded-xl text-xs font-bold hover:bg-blue-500">Cari</button>
                        <button onClick={() => { setShowGifPicker(false); setGifResults([]); setGifSearch(''); }} className="text-gray-400 hover:text-white p-2"><X size={16} /></button>
                      </div>
                      {gifLoading ? (
                        <div className="flex justify-center py-6"><div className="w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div>
                      ) : (
                        <div className="grid grid-cols-4 gap-2 max-h-48 overflow-y-auto scrollbar-thin">
                          {gifResults.map((gif, i) => (
                            <img
                              ...
                              alt="gif"
                              className="w-full h-16 object-cover rounded-xl cursor-pointer hover:opacity-80 transition-opacity border border-gray-800 hover:border-blue-500"
                              onClick={() => sendGif(gif.media_formats?.gif?.url)}
                            />
                          ))}
                          {gifResults.length === 0 && !gifLoading && (
                            <p className="col-span-4 text-center text-xs text-gray-600 py-4">Ketik lalu tekan Cari</p>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="p-4 bg-gray-950 border-t border-gray-800 z-10 flex-shrink-0 relative">
                    <form onSubmit={sendChat} className="flex items-center gap-2 bg-gray-900 border border-gray-800 p-1 rounded-2xl focus-within:border-blue-500 transition-colors">
                      ...
                      <button
                        type="button"
                        onClick={() => { setShowGifPicker(p => !p); if (!showGifPicker) searchGif(''); }}
                        className={`p-2 rounded-xl font-black text-[10px] uppercase transition-colors ml-1 ${showGifPicker ? 'bg-blue-600 text-white' : 'text-gray-500 hover:text-blue-400 hover:bg-gray-800'}`}
                      >GIF</button>
                      <input
                        required={!showGifPicker}
                        value={chatInput}
                        onChange={e => setChatInput(e.target.value)}
                        placeholder={`Kirim pesan ke ${(activeChatFriend?.name || 'teman').split(' ')[0]}...`}
                        className="flex-grow bg-transparent text-white px-2 py-3 text-sm outline-none"
                      />
      ...
      {bookDetailModal && (
        <div className="fixed inset-0 z-[150] bg-black/95 flex flex-col p-4 md:p-10 animate-fadeIn overflow-y-auto">
          <div className="max-w-4xl mx-auto w-full bg-gray-900 rounded-[2.5rem] border border-gray-800 shadow-xl relative my-auto overflow-hidden">
      ...
      {/* NOTIFIKASI */}
      {notification && (
        <div className={`fixed top-5 md:top-auto md:bottom-24 left-1/2 -translate-x-1/2 z-[400] px-6 py-3 md:px-8 md:py-4 rounded-full md:rounded-2xl shadow-xl animate-slideUp flex items-center gap-3 ${notification.type === 'error' ? 'bg-red-600 text-white' : 'bg-blue-600 text-white border border-blue-500'}`}>
