...
export default function App() {
  ...
  const handleLogin = async (e) => {
    ...
    // Payload selalu membawa password baik siswa maupun admin
    const payload = userType === 'admin' ? { is_admin: true, username: loginForm.username, password: loginForm.password } : { is_admin: false, nisn: loginForm.nisn.trim(), password: loginForm.password };
    try {
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              {userType === 'admin' ? (
                <div className="animate-fadeIn">
                  <input required placeholder="Username Admin" className="w-full p-4 bg-gray-800 text-white border border-gray-700 rounded-2xl outline-none focus:border-blue-500 mb-4" value={loginForm.username} onChange={e => setLoginForm(prev => ({...prev, username: e.target.value}))} />
                  <input required type="password" placeholder="Password Admin" className="w-full p-4 bg-gray-800 text-white border border-gray-700 rounded-2xl outline-none font-mono focus:border-blue-500" value={loginForm.password} onChange={e => setLoginForm(prev => ({...prev, password: e.target.value}))} />
                </div>
              ) : (
                <div className="animate-fadeIn">
                  <div className="bg-gray-800 p-4 rounded-2xl mb-4 border border-gray-700">
                      <p className="text-[10px] text-gray-400 text-center leading-relaxed">Masukkan NISN dan buat/masukkan Password kamu. Pastikan Admin sudah mendaftarkan NISN kamu di Server.</p>
                  </div>
                  <input required placeholder="Masukkan NISN" className="w-full p-5 mb-4 bg-gray-950 text-white border-2 border-gray-700 rounded-2xl outline-none focus:border-blue-500 font-mono text-center text-lg tracking-widest placeholder-gray-700" value={loginForm.nisn} onChange={e => setLoginForm(prev => ({...prev, nisn: e.target.value}))} />
                  <input required type="password" placeholder="Password Kamu" className="w-full p-4 bg-gray-800 text-white border border-gray-700 rounded-2xl outline-none focus:border-blue-500 text-center" value={loginForm.password} onChange={e => setLoginForm(prev => ({...prev, password: e.target.value}))} />
                </div>
              )}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                {(books || []).filter(b => b.status === 'approved').map(book => {
                  const hasCover = book.cover && typeof book.cover === 'string' && book.cover.trim().length > 20;
                  return (
                    ...
                    <form onSubmit={async (e) => {
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-4 bg-gray-950 border border-gray-800 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 transition-colors relative h-32">
                                ...
                                {bookForm.coverBase64 ? <img src={bookForm.coverBase64} onError={handleImgErr} className="h-full object-cover rounded-xl"/> : <><Camera size={24} className="text-gray-500 mb-2"/><span className="text-[10px] font-bold text-gray-500 uppercase text-center">Cover Gambar<br/>(Opsional)</span></>}
                            </div>
                            <div className="p-4 bg-gray-950 border border-gray-800 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 transition-colors relative h-32">
                                        ...
                                        <div key={idx} className={`flex gap-3 items-end ${isMe ? 'justify-end' : 'justify-start'}`}>
                                            ...
                                            <div className="flex flex-col group relative max-w-[75%] md:max-w-md">
                                                <div className={`p-3 px-4 rounded-2xl text-sm shadow-sm ${isMe ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-gray-800 text-gray-200 border border-gray-700 rounded-bl-sm'}`}>
                                                    {msg.type === 'gif' ? (
                                                        <img src={msg.text} onError={handleImgErr} className="w-48 rounded-xl object-cover" />
                                                    ) : (
                ...
                <div className="bg-gray-900 rounded-[3rem] shadow-xl text-center border border-gray-800 relative overflow-hidden flex flex-col">
                    ...
                    <div className="px-8 pb-10 pt-0 relative flex-grow">
                        ...
                        <h2 className={`text-3xl font-black uppercase flex items-center justify-center gap-2 ${myAchs.glowName ? 'text-white drop-shadow-[0_0_10px_rgba(59,130,246,0.8)]' : 'text-white'}`}>
                            {(user?.name || 'User')} {myAchs.socialTitle && <Award size={20} className="text-blue-400 drop-shadow-[0_0_5px_rgba(96,165,250,0.8)]" title="Sosialita SMANSA"/>}
                        </h2>
                        {user.level !== 'Admin' ? (
             ...
             <div className="max-w-4xl mx-auto w-full bg-gray-900 rounded-[2.5rem] border border-gray-800 shadow-[0_0_50px_rgba(0,0,0,0.8)] relative my-auto overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row gap-8 p-8 relative z-10">
                    <div className="w-full md:w-1/3 flex flex-col gap-4">
                        <div className="relative">
                            <div className="w-full rounded-2xl shadow-xl border border-gray-800 overflow-hidden flex items-center justify-center">
                                <SafeAvatar src={bookDetailModal.cover} sizeClassName="w-full h-auto min-h-[16rem]" iconSize={64}/>
                            </div>
                            {/* Wishlist Buttons */}
                            {user.level !== 'Admin' && (
